...
const getCampaign = async function getCampaign(req, res, next) {
  try {
    ...
    //let campaign = await Campaign.findById(req._userParams.campaignId).exec();
    res.send(201, {
      message: "Campaign successfully retrieved",
      data: campaign,
    });
  } catch (err) {
    handleError(err, next);
...
const updateCampaign = async function updateCampaign(req, res, next) {
  try {
    let campaign = await Campaign.findOneAndUpdate(
      { _id: req._userParams.campaignId },
      req._userParams
    ).exec();
...
const deleteCampaign = async function deleteCampaign(req, res, next) {
  try {
    let deletedCampaign = await Campaign.remove({
      _id: req._userParams.campaignId,
    }).exec();
...
module.exports = {
  postNewCampaign: [
    ...
    retrieveParameters([
      ...
      { name: "participantCount", dataType: "Number" },
  ...
  getCampaign: [
    retrieveParameters([{ name: "campaignId", required: true }]),
    getCampaign,
  ],
  updateCampaign: [
    checkContentType,
    retrieveParameters([
      ...
      { name: "description", required: true, datatType: "String" },
      { name: "priority", required: true, dataType: "String" },
      { name: "location", required: true, dataType: "String" },
      { name: "participantCount", required: false, dataType: "Number" },
